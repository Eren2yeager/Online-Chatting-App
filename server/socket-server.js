    ...
    try {
      ...
      await Chat.findByIdAndUpdate(chatId, {
        lastMessage: {
          ...
          type: media && media.length > 0 ? (media[0].mime && media[0].mime.startsWith('image/') ? 'image' : 'file') : 'text',
      ...
      const offlineUsers = chat.participants.filter(
        participant => !userSockets.has(participant._id.toString()) && 
  ...
  socket.on('reaction:add', async (data) => {
    try {
      ...
      const existingReaction = message.reactions.find(
        r => r.emoji === emoji && r.by.toString() === socket.userId
      );
