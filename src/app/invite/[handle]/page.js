...
export default function InviteHandlePage() {
  ...
  const [requestStatus, setRequestStatus] = useState(null); // 'pending', 'sent', 'error'
  ...
  const checkExistingRelationship = async (targetUserId) => {
    try {
      ...
      if (friendsRes.ok) {
        ...
        isFriend = Array.isArray(friends) && friends.some(friend => {
          if (typeof friend === "string") {
            return friend === targetUserId;
          } else if (friend && (friend._id || friend.id)) {
            return (friend._id || friend.id) === targetUserId;
          }
          return false;
        });
        if (isFriend) {
