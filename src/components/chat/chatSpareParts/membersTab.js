...
export default function MembersTab({
    ...
    const isAdminUser = (userId) => (admins?.length ? admins.some((a) => a._id === userId) : (chat?.admins || []).some((a) => a._id === userId));

    // const promote = async (userId) => {
    //   if (!isCreator && !isAdmin) return;
    //   await fetch(onPromoteDemoteApi?.promoteEndpoint || `/api/chats/${chat._id}/admins`, {
    //     method: "POST",
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          {(isAdmin || chat?.privacy === "member_invite") && (
            <button onClick={() => setAddDialogOpen(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
          ...
          {participants.map((p) => {
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {isCreator && !isCreatorFlag && (
                    isAdminFlag ? (
                      <button onClick={() => handleDemoteAdmin(p._id)} disabled={loading} className="px-2 py-1 text-xs rounded-md text-red-700 border border-red-200 hover:bg-red-50">Demote</button>
                    ) : (
                      <button onClick={() => handlePromoteAdmin(p._id)} disabled={loading} className="px-2 py-1 text-xs rounded-md text-green-700 border border-green-200 hover:bg-green-50">Promote</button>
                    )
                  )}
        ...
        {(isAdmin || chat?.privacy === "member_invite") && addDialogOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setAddDialogOpen(false)}>
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-4" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-md font-semibold text-gray-800">Add friends</h4>
                <button onClick={() => setAddDialogOpen(false)} className="p-2 rounded hover:bg-gray-100"><XMarkIcon className="h-5 w-5 text-gray-500"/></button>
              </div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {friends.map((f) => {
                  ...
                  return (
                    <label key={f._id} className={`flex items-center justify-between p-2 rounded-lg border ${isMember ? 'border-gray-200 bg-gray-50' : 'border-gray-200'}`}>
                      <div className="flex items-center space-x-3">
                        <input type="checkbox" disabled={isMember} checked={checked} onChange={() => toggleSelect(f._id)} className="h-4 w-4" />
                      ...
                      {isMember && <span className="text-xs text-gray-400">Already member</span>}
                    </label>
                  );
                })}
              </div>
              ...
              <div className="mt-4 flex justify-end space-x-2">
                <button onClick={() => setAddDialogOpen(false)} className="px-3 py-2 rounded border">Cancel</button>
                <button onClick={confirmAddMembers} disabled={loading || selectedIds.size === 0} className="px-3 py-2 rounded bg-blue-600 text-white disabled:opacity-50">Add {selectedIds.size > 0 ? `(${selectedIds.size})` : ''}</button>
              </div>
            </div>
          </div>
        )}
